...
async function performAutofill(highlightFilled) {
  ...
  for (const field of fields) {
    ...
    if (fieldType === "firstName") {
      // Use stored firstName if available, otherwise parse from fullName
      value =
        profile.firstName ||
        window.FieldMatcher.splitName(profile.fullName || "").firstName;
    } else if (fieldType === "lastName") {
      // Use stored lastName if available, otherwise parse from fullName
      value =
        profile.lastName ||
        window.FieldMatcher.splitName(profile.fullName || "").lastName;
    } else {
